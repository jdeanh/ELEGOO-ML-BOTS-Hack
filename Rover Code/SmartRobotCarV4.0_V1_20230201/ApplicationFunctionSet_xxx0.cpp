...
/*f(x) int */
static boolean
function_xxx(long x, long s, long e) //f(x)R
{
  if (s <= x && x <= e)
    return true;
  else
...
enum SmartRobotCarFunctionalModel
{
  ...
  Viper_mode,             /* JH - ML-BOTS Viper Mode*/
  CMD_CarControl_TimeLimit_Viper,
...
static void ApplicationFunctionSet_SmartRobotCarMotionControl(SmartRobotCarMotionControl direction, uint8_t is_speed)
{
  ...
  case CMD_CarControl_NoTimeLimit:
    ...
    UpperLimit = 180;
    break;
  case Viper_mode: /* JH - ML-BOTS Viper Mode */
    Kp = 10;
    UpperLimit = 200;
    break;
  case CMD_CarControl_TimeLimit_Viper:
    Kp = 10;
    UpperLimit = 200;
    break;
  default:
...
void ApplicationFunctionSet::ApplicationFunctionSet_RGB(void)
{
  ...
  {
    //Serial.print("Functional_Mode=");
    //Serial.println(Application_SmartRobotCarxxx0.Functional_Mode); // Convert ping time to distance and print result (0 = outside set distance range, no ping echo)
    ...
    case /* constant-expression */ Rocker_mode:
      ...
      break;
    case /* constant-expression */ Viper_mode:  /* JH - ML-BOTS Viper Mode */
      /* code */
      {
        AppRBG_LED.DeviceDriverSet_RBGLED_xxx(0 /*Duration*/, 2 /*Traversal_Number*/, CRGB::Cyan);
      }
    default:
      break;
    }
...
void ApplicationFunctionSet::ApplicationFunctionSet_Obstacle(void)
{
  ...
  if (Application_SmartRobotCarxxx0.Functional_Mode == ObstacleAvoidance_mode)
  {
    ...
    if (function_xxx(get_Distance, 0, 20)) //check to see if the ultrasonic sensor sees something within 0 and 20 cm
    {
      ApplicationFunctionSet_SmartRobotCarMotionControl(stop_it, 0);
...
void ApplicationFunctionSet::ApplicationFunctionSet_KeyCommand(void)
{
  ...
  if (temp_keyValue != get_keyValue)
  {
    ...
    case /* constant-expression */ 5:
      /* code */
      Application_SmartRobotCarxxx0.Functional_Mode = Viper_mode;
    default:
...
void ApplicationFunctionSet::ApplicationFunctionSet_IRrecv(void)
{
  ...
  if (true == IRrecv_en)
  {
    ...
    break;
    case /* constant-expression */ 13: // JH - added ultrasonic print output and Viper_mode button
    {
      Rocker_CarSpeed = 250;
    }
  ...
  if (c == '}') //Data frame tail check
  {
    ...
    else if (!error) //Check if the deserialization is successful
    {
      ...
      case 6:                                                                          /*<Command：N 6> */
        ...
        mpu.getMotion6(&ax, &ay, &az, &gx, &gy, &gz); /* JH - get MPU data */
#if _is_print
        char toString[50];
      ...
      case 9: /*<Command: N 9> */ /* JH - ML-Bots Viper */
        Application_SmartRobotCarxxx0.Functional_Mode = CMD_CarControl_TimeLimit_Viper;
        break;
      ...
      case 101: /*<Command：N 101> :remote control to switch the car mode*/
        ...
        else if (4 == doc["D1"]) /* JH - ML-BOTS Viper Mode*/
        {
          Application_SmartRobotCarxxx0.Functional_Mode = Viper_mode;
        }
      ...
      // case 103: /*<Command: N 103>: Viper control mode command */  /* JH - ML-BOTS Viper Mode*/
      //   Application_SmartRobotCarxxx0.Functional_Mode = Viper_mode;
      //   Viper_temp = doc["D1"];
      //   Viper_CarSpeed = doc["D2"];
...
void ApplicationFunctionSet::ApplicationFunctionSet_Viper(void) /* JH - ML-BOTS Viper Mode*/
{
  static boolean first_is = true;
  uint8_t switc_ctrl = 0;
  uint16_t get_Distance;
  if (Application_SmartRobotCarxxx0.Functional_Mode == Viper_mode)
  {
    /* react to AS7341 and BME680 data here*/
...
void ApplicationFunctionSet::CMD_CarControlTimeLimit_Viper_xxx0(void) /* JH - ML-BOTS Viper */
{
  static boolean CarControl = false;
  static boolean CarControl_TE = false; //Time stamp
  static boolean CarControl_return = false;
  ...
  uint8_t switc_ctrl = 0;
  uint16_t get_Distance;
  if (Application_SmartRobotCarxxx0.Functional_Mode == CMD_CarControl_TimeLimit_Viper) //enter time-limited control mode
  {
    ...
    {
      AppULTRASONIC.DeviceDriverSet_ULTRASONIC_Get(&get_Distance /*out*/);
      if (function_xxx(get_Distance, 0, 20) && CMD_is_CarDirection == Forward) //check to see if the ultrasonic sensor sees something within 0 and 20 cm
      {
          ApplicationFunctionSet_SmartRobotCarMotionControl(stop_it, 0);
          Application_SmartRobotCarxxx0.Functional_Mode = Viper_mode; /*set mode to viper mode<Waiting for next set of control commands>*/
      }
      if ((millis() - Application_SmartRobotCarxxx0.CMD_CarControl_Millis) > (CMD_is_CarTimer)) //check the timestamp
      {
        CarControl_TE = true;
        ...
        Application_SmartRobotCarxxx0.Functional_Mode = Viper_mode; /*set mode to viper mode<Waiting for the next set of control commands>*/
        if (CarControl_return == false)
        {
